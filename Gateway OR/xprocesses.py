...
def run_round(paramenter_set, number_of_islands, round, broadcast, messenger, island_sizes, percentage_of_best_individuals_for_migration_all_islands, full_log, alphabet, log_name, xes_log, max_number_of_generations, max_processing_time, stop_condition, bestone_file, cache_fitness, cache_soundness, cache_petri_net, cache_n_tokens, fitness_lock, soundness_lock, petri_net_lock, n_tokens_lock, island_number):
    ...
    pn.create_pn(population[int(highest_value_and_position[1])], alphabet, island_number, 0 , log_name, round, cache_petri_net, petri_net_lock)
    ...
    for current_generation in range(1, max_number_of_generations):
        ...
        pn.create_pn(population[int(highest_value_and_position[1])], alphabet, island_number, current_generation, log_name, round, cache_petri_net, petri_net_lock)
        island_end = dt.datetime.now()
        island_duration = island_end - island_start
        ...
        if (current_generation > 0) and (current_generation % migration_time == 0):
            ...
            isl.send_individuals(population, island_number, number_of_islands, sorted_evaluated_population, messenger)
            (highest_value_and_position, sorted_evaluated_population) = cyc.choose_highest(evaluated_population)
        rec.record_evolution(log_name, str(round), paramenter_set[island_number + 1], island_number, highest_value_and_position[0], fitness_evolution, alphabet, population[int(highest_value_and_position[1])], island_start, island_end, island_duration, current_generation)
...
if __name__ == '__main__':
    ...
    for round in range(0, number_of_rounds):
        ...
        fitness_lock = m.Lock()
        soundness_lock = m.Lock()
        petri_net_lock = m.Lock()
        n_tokens_lock = m.Lock()
