...
def run_round(paramenter_set, number_of_islands, round, broadcast, messenger, island_sizes, percentage_of_best_individuals_for_migration_all_islands, algo_option, algo_option_changed, full_log, alphabet, log_name, xes_log, max_number_of_generations, max_processing_time, stop_condition, bestone_file, island_number):
    ...
    for current_generation in range(1, max_number_of_generations):
        ...
        if (algo_option[0] != 'ALIGNMENT_BASED-ALIGN_ETCONFORMANCE') and (fitness_evolution[current_generation][8] >= (stop_condition * 2 / 5)):
            algo_option[0] = 'ALIGNMENT_BASED-ALIGN_ETCONFORMANCE'
        if (algo_option[0] == 'ALIGNMENT_BASED-ALIGN_ETCONFORMANCE') and (algo_option_changed[island_number] == 0):
    ...
    # print('Final results ==>', 'r', round, '| ISL:', island_number, '| ISL-DURATION:', island_duration, '| HM:', '%.5f' % highest_value_and_position[0][0], '| FIT:', '%.5f' % highest_value_and_position[0][1], '| PREC:', '%.5f' % highest_value_and_position[0][2], '| GEN:', '%.5f' % highest_value_and_position[0][3], '| SIMP:', '%.5f' % highest_value_and_position[0][4])
...
if __name__ == '__main__':
        for round in range(0, number_of_rounds):
            ...
            broadcast.append(1)
            algo_option.append('ALIGNMENT_BASED-ALIGN_ETCONFORMANCE')
