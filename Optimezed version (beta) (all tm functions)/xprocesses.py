...
def calculate_statistics(evaluated_population):
    highest_value_and_position, sorted_evaluated_population = cyc.choose_highest_tm(evaluated_population)
    lowest_value = cyc.choose_lowest_tm(sorted_evaluated_population)
    average_value = cyc.calculate_average_tm(evaluated_population)
...
def run_round(paramenter_set, number_of_islands, round, broadcast, messenger, island_sizes, percentage_of_best_individuals_for_migration_all_islands, algo_option, algo_option_changed, full_log, alphabet, log_name, xes_log, max_number_of_generations, max_processing_time, stop_condition, bestone_file, island_number):
    ...
    population, evaluated_population, reference_cromossome = inp.initialize_population_tm(population_size, alphabet, full_log, xes_log, algo_option[0], fitness_weight, precision_weight, generalization_weight, simplicity_weight)
    fitness_evolution = []
    ...
    pn.create_pn_tm(population[int(highest_value_and_position[1])], alphabet, island_number, 0 , log_name, 0)
    ...
    isl.set_broadcast_tm(population, sorted_evaluated_population, island_number, percentage_of_best_individuals_for_migration_per_island, broadcast)
    if isl.receive_individuals_tm(population, island_number, sorted_evaluated_population, messenger, evaluated_population) == 1:
        (highest_value_and_position, sorted_evaluated_population) = cyc.choose_highest_tm(evaluated_population)
    if migration_time == 1:
        island_fitness = [evaluated_population[1][i][0] for i in range(len(evaluated_population[1]))]
        isl.do_migration_tm(population, island_number, number_of_islands, island_fitness, percentage_of_individuals_for_migration_per_island, broadcast, island_sizes, percentage_of_best_individuals_for_migration_all_islands, evaluated_population)
        isl.send_individuals_tm(population, island_number, number_of_islands, sorted_evaluated_population, messenger)
        highest_value_and_position, sorted_evaluated_population = cyc.choose_highest_tm(evaluated_population)
    rec.record_evolution_tm(log_name, str(round), paramenter_set[island_number + 1], island_number, highest_value_and_position[0], fitness_evolution, alphabet, population[int(highest_value_and_position[1])], island_start, island_end, island_duration, 0, algo_option[0])
    for current_generation in range(1, max_number_of_generations):
        (population, evaluated_population) = cyc.generation_tm(population, reference_cromossome, evaluated_population, crossover_probability, max_perc_of_num_tasks_for_crossover, task_mutation_probability, gateway_mutation_probability, max_perc_of_num_tasks_for_task_mutation, max_perc_of_num_tasks_for_gateway_mutation, elitism_percentage, sorted_evaluated_population, alphabet, xes_log, algo_option[0], fitness_weight, precision_weight, generalization_weight, simplicity_weight)
        highest_value_and_position, lowest_value, average_value, sorted_evaluated_population = calculate_statistics(evaluated_population)
        fitness_evolution.append([lowest_value, highest_value_and_position[0][0], average_value, 0, highest_value_and_position[0][1], highest_value_and_position[0][2], highest_value_and_position[0][3], highest_value_and_position[0][4], 0, 0, 0, 0])
        ...
        pn.create_pn_tm(population[int(highest_value_and_position[1])], alphabet, island_number, current_generation, log_name, round)
        island_end = dt.datetime.now()
        island_duration = island_end - island_start
        ...
        if (current_generation > 0) and (current_generation % migration_time == 0):
            ...
            isl.send_individuals_tm(population, island_number, number_of_islands, sorted_evaluated_population, messenger)
            (highest_value_and_position, sorted_evaluated_population) = cyc.choose_highest_tm(evaluated_population)
        rec.record_evolution_tm(log_name, str(round), paramenter_set[island_number + 1], island_number, highest_value_and_position[0], fitness_evolution, alphabet, population[int(highest_value_and_position[1])], island_start, island_end, island_duration, current_generation, algo_option[0])
        ...
        if (algo_option[0] == 'ALIGNMENT_BASED-ETCONFORMANCE_TOKEN') and (algo_option_changed[island_number] == 0):
            evaluated_population = fit.evaluate_population_tm(population, alphabet, xes_log, algo_option[0], fitness_weight, precision_weight, generalization_weight, simplicity_weight)
            (highest_value_and_position, sorted_evaluated_population) = cyc.choose_highest_tm(evaluated_population)
            for island_n in range(0, number_of_islands):
                isl.set_broadcast_null_tm(island_n, broadcast)
                isl.send_individuals_null_tm(island_n, messenger)
            algo_option_changed[island_number] = 1
    evaluated_final_population = fit.evaluate_population_tm(population, alphabet, xes_log, 'ALIGNMENT_BASED-ALIGN_ETCONFORMANCE', fitness_weight, precision_weight, generalization_weight, simplicity_weight)
    ...
    plt.plot_evolution_per_island_tm(fitness_evolution, log_name, str(round), island_number)
    with open('output-graphs/plotting/plotting_{0}.txt'.format(island_number), 'r') as plott:
        previous_plotting = [literal_eval(line) for line in isl.nonblank_lines_tm(plott)]
    ...
    rec.record_evolution_tm(log_name, str(round), paramenter_set[island_number + 1], island_number, highest_value_and_position[0], fitness_evolution, alphabet, population[int(highest_value_and_position[1])], island_start, island_end, island_duration, current_generation, 'ALIGNMENT_BASED-ALIGN_ETCONFORMANCE')
    rec.record_bestone_tm(island_number, highest_value_and_position[0][0], current_generation, bestone_file, island_duration)

if __name__ == '__main__':
        print(dt.datetime.now())
        paramenter_set = []
        with open('input-parameters/input-parameters.csv', 'r') as parameters:
            for line in isl.nonblank_lines_tm(parameters):
                paramenter_set.append(line.split(';'))
        full_log, log_name, xes_log = log.import_log_tm(inputlog)
        print(dt.datetime.now())
        # o trecho acima não tem otimização (até onde sei)
        ...
        for round in range(0, number_of_rounds):
            ...
            alphabet = inp.create_alphabet_tm(full_log)
            inp.process_log_tm(full_log)
            island_ids = list(range(number_of_islands))
            isl.create_plotting_files_tm(island_ids, number_of_islands)
            ...
            plt.plot_evolution_integrated_tm(log_name, str(round), number_of_islands)
            ...
            #printing results and shooting down...
            ...
            pn.write_and_show_best_pn_tm(best_island_number, log_name, round, generation_best, bestone_file)
            os.remove(bestone_file)
