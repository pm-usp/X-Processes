...
def evaluate_population(island_number, population, alphabet, xes_log, algo_option, SOLVER_LIMIT):
    evaluation_sum = 0                                                                                                  
    evaluation_values = []                                                                                              
    for i in range(len(population)):
        evaluation_values.append(evaluate_individual(island_number, population[i], alphabet, xes_log, i, algo_option, SOLVER_LIMIT))
        evaluation_sum = evaluation_sum + evaluation_values[i][0]                                                       
    return [evaluation_sum, evaluation_values]                                                                            


def evaluate_individual(island_number, cromossome, alphabet, xes_log, i, algo_option, SOLVER_LIMIT):
    fitness_weight = 1    
    precision_weight = 0
    generalization_weight = 0
    simplicity_weight = 0
