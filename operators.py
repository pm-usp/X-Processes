...
def crossover_per_process(crossover_probability, max_perc_of_num_tasks_for_crossover, cromossome_1, cromossome_2):          
    if ran.random() < crossover_probability:                                                                            
        ...
        if max_perc_of_num_tasks_for_crossover == -1:                                                                       
            ...
            number_of_tasks = int(max_perc_of_num_tasks_for_crossover * (len(cromossome_1) - 1))                            
        ...
        number_of_produced_tokens = count_produced_tokens(cromossome_1)
        number_of_consumed_tokens = count_consumed_tokens(cromossome_1)
        if number_of_produced_tokens > number_of_consumed_tokens:
            increase_number_of_consumed_tokens(cromossome_1, number_of_produced_tokens)
        elif number_of_produced_tokens < number_of_consumed_tokens:
            increase_number_of_produced_tokens(cromossome_1, number_of_produced_tokens)

        number_of_produced_tokens = count_produced_tokens(cromossome_2)
        number_of_consumed_tokens = count_consumed_tokens(cromossome_2)
        if number_of_produced_tokens > number_of_consumed_tokens:
            increase_number_of_consumed_tokens(cromossome_2, number_of_produced_tokens)
        elif number_of_produced_tokens < number_of_consumed_tokens:
            increase_number_of_produced_tokens(cromossome_2, number_of_produced_tokens)
...
def task_mutation(cromossome, max_perc_of_num_tasks_for_task_mutation, reference_cromossome):
    ...
    for i in range(number_of_tasks):
        ...
        if chosen_mutation_type < 1/2:
            ...
            number_of_produced_tokens = count_produced_tokens(cromossome)
            number_of_consumed_tokens = count_consumed_tokens(cromossome)
            if number_of_produced_tokens > number_of_consumed_tokens:
                increase_number_of_consumed_tokens(cromossome, number_of_produced_tokens)
            elif number_of_produced_tokens < number_of_consumed_tokens:
                increase_number_of_produced_tokens(cromossome, number_of_produced_tokens)
        else:
            for j in range(len(cromossome)):
            ...
            if number_of_consumed_tokens > number_of_produced_tokens:
                increase_number_of_produced_tokens(cromossome, number_of_consumed_tokens)
            elif number_of_consumed_tokens < number_of_produced_tokens:
                increase_number_of_consumed_tokens(cromossome, number_of_consumed_tokens)
    return
...
def count_produced_tokens(cromossome):
    number_of_produced_tokens = 0
    for i in range(len(cromossome) - 2):
        if cromossome[i][-1] == 0:
            number_of_produced_tokens = number_of_produced_tokens + 1
        else:
            for j in range(1, len(cromossome[i]) - 1):
                number_of_produced_tokens = number_of_produced_tokens + cromossome[i][j]
    return number_of_produced_tokens


def count_consumed_tokens(cromossome):
    number_of_consumed_tokens = 0
    for i in range(1, len(cromossome) - 1):
        if cromossome[-1][i] == 0:
            number_of_consumed_tokens = number_of_consumed_tokens + 1
        else:
            for j in range(len(cromossome[i]) - 2):
                number_of_consumed_tokens = number_of_consumed_tokens + cromossome[j][i]
    return number_of_consumed_tokens


def increase_number_of_produced_tokens(cromossome, number_of_consumed_tokens):
    ...
    for i in range(number_of_attempts):
        ...
        while cromossome[chosen_task][-1] == 1:
            chosen_task = ran.randint(0, int(len(cromossome) - 3))
        if is_there_at_least_two_raw_active_tasks(cromossome, chosen_task):
            cromossome[chosen_task][-1] = 1
        number_of_produced_tokens = count_produced_tokens(cromossome)
        while number_of_consumed_tokens > number_of_produced_tokens:
            chosen_task = ran.randint(0, int(len(cromossome) - 3))
            while cromossome[chosen_task][-1] == 1:
                chosen_task = ran.randint(0, int(len(cromossome) - 3))
            if is_there_at_least_two_raw_active_tasks(cromossome, chosen_task):
    ...
    if i == number_of_attempts - 1:
        decrease_number_of_consumed_tokens(cromossome, count_produced_tokens(cromossome))
    return


def increase_number_of_consumed_tokens(cromossome, number_of_produced_tokens):
    copy_of_input_gateways = copy.deepcopy(cromossome[-1])
    number_of_attempts = int(len(cromossome) * 0.5)
    for i in range(number_of_attempts):
        chosen_task = ran.randint(1, int(len(cromossome) - 2))
        while cromossome[-1][chosen_task] == 1:
            chosen_task = ran.randint(1, int(len(cromossome) - 2))
        if is_there_at_least_two_column_active_tasks(cromossome, chosen_task):
            cromossome[-1][chosen_task] = 1
        number_of_consumed_tokens = count_consumed_tokens(cromossome)
        while number_of_produced_tokens > number_of_consumed_tokens:
            chosen_task = ran.randint(1, int(len(cromossome) - 2))
            while cromossome[-1][chosen_task] == 1:
                chosen_task = ran.randint(1, int(len(cromossome) - 2))
            if is_there_at_least_two_column_active_tasks(cromossome, chosen_task):
    ...
    if i == number_of_attempts - 1:
        decrease_number_of_produced_tokens(cromossome, count_consumed_tokens(cromossome))
    return


def decrease_number_of_produced_tokens(cromossome, number_of_consumed_tokens):
    copy_of_output_gateways = []
    for i in range(len(cromossome)):
        copy_of_output_gateways.append(cromossome[i][-1])
    number_of_attempts = int(len(cromossome) * 0.5)
    for i in range(number_of_attempts):
        chosen_task = ran.randint(0, int(len(cromossome) - 3))
        while cromossome[chosen_task][-1] == 0:
            chosen_task = ran.randint(0, int(len(cromossome) - 3))
        cromossome[chosen_task][-1] = 0
        number_of_produced_tokens = count_produced_tokens(cromossome)
        while number_of_produced_tokens > number_of_consumed_tokens:
...
def decrease_number_of_consumed_tokens(cromossome, number_of_produced_tokens):
    copy_of_input_gateways = copy.deepcopy(cromossome[-1])
    number_of_attempts = int(len(cromossome) * 0.5)
    for i in range(number_of_attempts):
        chosen_task = ran.randint(1, int(len(cromossome) - 2))
        while cromossome[-1][chosen_task] == 0:
            chosen_task = ran.randint(1, int(len(cromossome) - 2))
        cromossome[-1][chosen_task] = 0
        number_of_consumed_tokens = count_consumed_tokens(cromossome)
        while number_of_consumed_tokens > number_of_produced_tokens:
...
def elitism(population, elitism_percentage, sorted_evaluated_aux_population, sorted_evaluated_population, auxiliary_population):
