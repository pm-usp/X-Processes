...
@decorators.measure_time
def crossover_per_process(crossover_probability, max_perc_of_num_tasks_for_crossover, cromossome_1, cromossome_2, reference_cromossome, cache_n_tokens):
    if ran.random() < crossover_probability:
        ...
        if all(checks):
            adjust_produced_consumed_tokens(offspring_1, cache_n_tokens)
            adjust_produced_consumed_tokens(offspring_2, cache_n_tokens)
            return offspring_2, offspring_1
    return cromossome_1, cromossome_2

@decorators.measure_time
def mutation(cromossome, task_mutation_probability, gateway_mutation_probability, max_perc_of_num_tasks_for_task_mutation, max_perc_of_num_tasks_for_gateway_mutation, reference_cromossome, island, on_off_task_mutation_probability, max_perc_of_num_tasks_for_on_off_task_mutation, cache_n_tokens, act_fix_mode):
    if ran.random() < task_mutation_probability:
        task_mutation(cromossome, max_perc_of_num_tasks_for_task_mutation, reference_cromossome, island, cache_n_tokens)
    if ran.random() < gateway_mutation_probability:
        gateway_mutation(cromossome, max_perc_of_num_tasks_for_gateway_mutation, cache_n_tokens)
    if ran.random() < on_off_task_mutation_probability:
        if not act_fix_mode:
            on_off_task_mutation(cromossome, reference_cromossome, max_perc_of_num_tasks_for_on_off_task_mutation, cache_n_tokens)
...
@decorators.measure_time
def count_number_of_tokens(cromossome, cache_n_tokens):
...
@decorators.measure_time
def task_mutation(cromossome, max_perc_of_num_tasks_for_task_mutation, reference_cromossome, island, cache_n_tokens):
    ...
    while True:
        ...
        for _ in range(number_of_tasks):
            ...
            adjust_produced_consumed_tokens(cromossome, cache_n_tokens)
...
@decorators.measure_time
def on_off_task_mutation(cromossome, reference_cromossome, max_perc_of_num_tasks_for_on_off_task_mutation, cache_n_tokens):
    ...
    if ran.random() < 0.5:
        ...
        for task in tasks_to_activate:
            cromossome = activate_task(cromossome, reference_cromossome, task, cache_n_tokens)
    else:
        ...
        for task in tasks_to_deactivate:
            cromossome = deactivate_task(cromossome, reference_cromossome, task, cache_n_tokens)
    return cromossome

@decorators.measure_time
def activate_task(cromossome, reference_cromossome, task_index, cache_n_tokens):
...
@decorators.measure_time
def deactivate_task(cromossome, reference_cromossome, task_index, cache_n_tokens):
...
@decorators.measure_time
def gateway_mutation(cromossome, max_perc_of_num_tasks_for_gateway_mutation, cache_n_tokens):
    ...
    if max_perc_of_num_tasks_for_gateway_mutation == -1:
        ...
        number_of_tasks = ran.randint(1, max_number_of_tasks)
    for _ in range(number_of_tasks):
        mutate_and_adjust(cromossome, len_cromossome, cache_n_tokens)

@decorators.measure_time
def mutate_and_adjust(cromossome, len_cromossome, cache_n_tokens):
