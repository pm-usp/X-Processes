...
@decorators.measure_time
def create_alphabet(listOfLists_log):
    alphabet_set = set()
    for trace in listOfLists_log:
...
@decorators.measure_time
def process_log(listOfLists_log):
    listOfLists_log = [process_entry(entry) for entry in listOfLists_log]
...
def initialize_population(population_size, alphabet, listOfLists_log, fitness_weight, precision_weight, generalization_weight, simplicity_weight, input_log_name, round, island, generation, cache_fitness, cache_soundness, cache_petri_net, cache_n_tokens, act_fix_mode, cached_sampled_xes_log, cached_remaining_xes_log, last_file_size_sampled_xes_log, last_file_size_remaining_xes_log):
    ...
    for i in range(population_size):
        population[i] = create_initial_individual(population[i], alphabet, reference_cromossome, cache_n_tokens, act_fix_mode)
        while not pn.is_sound(population[i], alphabet, input_log_name, round, island, generation, cache_soundness, cache_petri_net):
...
@decorators.measure_time
def create_DFG(listOfLists_log, alphabet):
...
@decorators.measure_time
def create_initial_individual(cromossome, alphabet, reference_cromossome, cache_n_tokens, act_fix_mode):
