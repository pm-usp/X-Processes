...
def run_round(paramenter_set, number_of_islands, round, broadcast, messenger, island_sizes, percentage_of_best_individuals_for_migration_all_islands, alphabet, input_log_name, max_number_of_generations, max_processing_time, stop_condition, bestone_file, cache_fitness, cache_soundness, cache_petri_net, cache_n_tokens, sampled_log_lock, remaining_log_lock, cached_sampled_xes_log, cached_remaining_xes_log, last_file_size_sampled_xes_log, last_file_size_remaining_xes_log, island_number):
    try:
        ...
        for current_generation in range(1, max_number_of_generations):
            ...
            isl.set_broadcast(population, sorted_evaluated_population, island_number, percentage_of_best_individuals_for_migration_per_island, broadcast)
            if ((fitness_evolution[current_generation][8] >= stop_condition/5)) or (island_duration > dt.timedelta(seconds=max_processing_time)):
...
if __name__ == '__main__':
    ...
    for round in range(0, number_of_rounds):
        ...
        bestone_file = 'petri-nets/' + input_log_name.replace("\\", "-").replace("/", "-") + time + '.txt'
        ...
        func2 = partial(func, round, broadcast, messenger, island_sizes, percentage_of_best_individuals_for_migration_all_islands, alphabet, input_log_name, max_number_of_generations, max_processing_time, stop_condition, bestone_file, cache_fitness, cache_soundness, cache_petri_net, cache_n_tokens, sampled_log_lock, remaining_log_lock, cached_sampled_xes_log, cached_remaining_xes_log, last_file_size_sampled_xes_log, last_file_size_remaining_xes_log)
        ...
        with open(bestone_file, 'r') as bestone:
            ...
            bestone.close()
        pn.write_and_show_best_pn(best_island_number, input_log_name, round, generation_best, bestone_file)
        os.remove(bestone_file)
        p.close()
        p.join()